...
public final class ExpressionBuilder implements NodeVisitor {
    ...
    private static class SynchronizedStack<T> {
        ...
        public static final int DEFAULT_SIZE = 128; //默认初始 对象数组大小
        private static final int DEFAULT_LIMIT = -1; //默认 不限制最大能存储的对象数量

        private int size; //当前 对象数组的大小
        private final int limit; //最大能存储的对象数量（构造函数传入）

        /*
         * Points to the next available object in the stack
         */
        private int index = -1; //当前可以使用的空闲对象的索引（其实就是对象数组中索引最大的那个空闲对象的索引）

        //内部维护一个对象数组,用数组实现栈的功能（为什么使用数组：用最低的内存和 GC 的代价来实现无界容器）
