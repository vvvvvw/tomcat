...
public class Http11InputBuffer implements InputBuffer, ApplicationBufferHandler {
    ...
    boolean parseRequestLine(boolean keptAlive, int connectionTimeout, int keepAliveTimeout)
        ...
        if (parsingRequestLinePhase < 2) {
            do {
                ...
                if (byteBuffer.position() >= byteBuffer.limit()) { //初始的时候 position和limit都是0
                    if (keptAlive) {
                        // Haven't read any request data yet so use the keep-alive
                        // timeout.
                        wrapper.setReadTimeout(keepAliveTimeout);
                    }
                    if (!fill(false)) { //这个地方就是 触发read操作的地方，对于nio：第一次read 注册completehandler，第二次read真正读取数据
    ...
    private boolean fill(boolean block) throws IOException {
        ...
        if (socketWrapper != null) {
            nRead = socketWrapper.read(block, byteBuffer); //调用 socketWrapper.read方法
        } else {
