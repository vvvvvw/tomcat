...
public class Nio2Endpoint extends AbstractJsseEndpoint<Nio2Channel,AsynchronousSocketChannel> {
    ...
    protected class Nio2Acceptor extends Acceptor<AsynchronousSocketChannel>
        ...
        public void run() {
            ...
            if (!isPaused()) {
                ...
                if (!isPaused()) {
                    ...
                    serverSock.accept(null, this); //accept连接，最终调用到 Nio2Acceptor（实现了CompletionHandler接口）的completed方法
                } else {
        ...
        public void completed(AsynchronousSocketChannel socket,
            ...
            if (isRunning() && !isPaused()) {
                if (getMaxConnections() == -1) { /*如果没有连接限制*/
                    serverSock.accept(null, this); //继续接收新的连接
                } else if (getConnectionCount() < getMaxConnections()) { //如果有连接限制，当前连接数小于 最大连接限制
                    try {
                        // This will not block
                        countUpOrAwaitConnection(); //调用LimitLatch的countUpOrAwait方法
                    } catch (InterruptedException e) {
                        // Ignore
                    }
                    serverSock.accept(null, this); //继续接收新的连接
                } else { //如果有连接限制，就在线程池里跑 run 方法去接收新的连接（其实 启动新线程的话连接数 也不会超过最大连接数，因为在 run方法中也有countUpOrAwaitConnection  ）
                    //为什么要放在线程池里跑呢？原因：如果放在当前线程里执行，completed 方法可能被阻塞，会导致这个回调方法一直不返回
    ...
    public static class Nio2SocketWrapper extends SocketWrapperBase<Nio2Channel> {
        ...
        private boolean readInterest = false; // Guarded by readCompletionHandler 猜测这个应该是用来 限流的，我也没有具体看使用的地方
        private boolean readNotify = false; //用来标志当前的socket是否能立即 读取到数据，对于 异步io来说 只有执行到 read回调函数的时候才能读取到数据
            ...
            this.readCompletionHandler = new CompletionHandler<Integer, ByteBuffer>() {
                ...
                public void completed(Integer nBytes, ByteBuffer attachment) {
                    ...
                    synchronized (readCompletionHandler) {
                        if (nBytes.intValue() < 0) { //如果调用了read回调方法，但是没有数据，则抛出异常关闭socket
                            failed(new EOFException(), attachment);
                        } else { //如果有数据
        ...
        public boolean isReadyForRead() throws IOException {
            synchronized (readCompletionHandler) {
                ...
                if (!readPending.tryAcquire()) { //如果读操作被 pending了，那么直到接收到 unpending的通知，否则都不能处理读操作
                    readInterest = true;
                    return false;
                }
        ...
        public int read(boolean block, byte[] b, int off, int len) throws IOException {
            ...
            int nRead = populateReadBuffer(b, off, len); //第二次read 通过这个方法取数据
            if (nRead > 0) {
            ...
            synchronized (readCompletionHandler) {
                ...
                nRead = fillReadBuffer(block); //第一次read数据没取到，会调用下面这个方法去真正执行I/O操作并注册回调函数：
