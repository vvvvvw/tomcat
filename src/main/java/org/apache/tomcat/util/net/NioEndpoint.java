...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel,SocketChannel> {
     ...
     */
    //ServerSocketChannel 是在多个 Acceptor 线程之间共享的，它是 Endpoint 的属性，由 Endpoint 完成初始化和端口绑定
    ...
    public class Poller implements Runnable {
        ...
        private Selector selector; //Poller管理的Selector
        //内部维护一个 Queue用来接收Acceptor传递过来的socket连接
        private final SynchronizedQueue<PollerEvent> events =
                new SynchronizedQueue<>();

        private volatile boolean close = false;
        // Optimize expiration handling
        private long nextExpiration = 0; //当前时间+设置的socket超时时间（socketProperties.getTimeoutInterval()）

        private AtomicLong wakeupCounter = new AtomicLong(0); //大于0 表示当前有任务要处理

        private volatile int keyCount = 0; //当前 准备好io的 SelectorKey的数量
         ...
         */
        //处理 Poller事件队列中的 event（如果是 OP_REGISTER事件，则把 读事件 注册到本Poller对应的Selector上；否则根据event中的interestops来重置 注册到SelectorKey中的interestops（和之前在SelectorKey中的interestops或一下））
        //
        public boolean events() {
            ...
            for (int i = 0, size = events.size(); i < size && (pe = events.poll()) != null; i++ ) {
                ...
                } else if (interestOps == OP_REGISTER) { //如果是刚注册的 socketchannel，则在poller对应的selector上注册读事件
                    try {
                    ...
                    if (key == null) {
                        ...
                        if (attachment != null) {
                            ...
                            try {
                                ...
                                key.interestOps(ops); //根据event中的 interestops来重置 interestOps
        ...
        public void register(final NioChannel socket, final NioSocketWrapper socketWrapper) {
            ...
            if (event == null) {
                //将 SocketChannel 对象封装在一个 PollerEvent 对象中，并将 PollerEvent 对象压入 Poller 的 Queue 里
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    if (!close) {
                        hasEvents = events();
                        if (wakeupCounter.getAndSet(-1) > 0) { //如果wakeupCounter的值不为0，则说明有事件处理
                            // If we are here, means we have other stuff to do
                            // Do a non blocking select
                            keyCount = selector.selectNow(); //则直接 非阻塞select
                        } else {
                            keyCount = selector.select(selectorTimeout); //否则，阻塞select
                        }
                        wakeupCounter.set(0);
                    }
                    if (close) {
                ...
                // any active event.
                while (iterator != null && iterator.hasNext()) { //处理
        ...
        protected void processKey(SelectionKey sk, NioSocketWrapper socketWrapper) {
            try {
                if (close) { //如果 服务端套接字关闭，则关闭所有的socket
                    ...
                    if (sk.isReadable() || sk.isWritable()) {
                        if (socketWrapper.getSendfileData() != null) {
                            ...
                            unreg(sk, socketWrapper, sk.readyOps()); //从 selector上注销 对应事件，这样的话就不会有多个线程同时 读或者写 同一个socket了
                            boolean closeSocket = false;
                            // Read goes before write
                            if (sk.isReadable()) {
                                if (socketWrapper.readOperation != null) {
                                    if (!socketWrapper.readOperation.process()) { //执行读操作
                                        closeSocket = true; //如果执行出错，则关闭socket
                                    }
                            ...
                            if (!closeSocket && sk.isWritable()) {
                                if (socketWrapper.writeOperation != null) {
                                    if (!socketWrapper.writeOperation.process()) { //执行写操作
                                        closeSocket = true; //如果执行出错，则关闭socket
                                    }
        ...
        /*
        执行条件：
        -selector只是简单的超时（意味着没有太多负载，keyCount <= 0 并且 hasEvents为false，表示没有事件要处理）
        -到了预定的超时事件（now < nextExpiration）
        -服务器套接字正在关闭
        执行的步骤：
        1.如果 服务器套接字正在关闭，则关闭所有的socket；否则
        2.如果发生了readTimeout（上一次读的事件距离现在 已经超过了 配置的读超时），则尝试一次读操作，如果报错，则关闭该socket；
        3.如果发生了writeTimeout（上一次写的事件距离现在 已经超过了 配置的写超时），则尝试一次写操作，如果报错，则关闭该socket；
         */
        protected void timeout(int keyCount, boolean hasEvents) {
            long now = System.currentTimeMillis();
            /*
            本方法会在Poller的每次循环上调用，但是不应该在Poller的每个循环上处理超时事件，因为这会产生过多的负载（超时可能需要等待几秒钟）。
            如果满足以下任一条件，请执行超时：
            -selector只是简单的超时（意味着没有太多负载，keyCount <= 0 并且 hasEvents为false，表示没有事件要处理）
            -到了预定的超时事件（now < nextExpiration）
            -服务器套接字正在关闭
            ...
            try {
                for (SelectionKey key : selector.keys()) {
                    ...
                    try {
                        if (socketWrapper == null) {
                        ...
                        } else if (close) { //如果close为true，则关闭所有socket
                            key.interestOps(0);
                            ...
                            if (readTimeout || writeTimeout) {//如果发生了readTimeout（上一次读的事件距离现在 已经超过了 读超时）或者writeTimeout（上一次写的事件距离现在 已经超过了 写超时）
                                key.interestOps(0);
                                // Avoid duplicate timeout calls
                                socketWrapper.interestOps(0);
                                socketWrapper.setError(new SocketTimeoutException());
                                if (readTimeout && socketWrapper.readOperation != null) {
                                    if (!socketWrapper.readOperation.process()) { //对这个socket执行一次读操作，如果发生了异常，则关闭这个socket
                                        cancelledKey(key, socketWrapper);
                                    }
                                } else if (writeTimeout && socketWrapper.writeOperation != null) {
                                    if (!socketWrapper.writeOperation.process()) {  //对这个socket执行一次写操作，如果发生了异常，则关闭这个socket
                                        cancelledKey(key, socketWrapper);
                                    }
     ...
     */
    //SocketProcessor 实现了 Runnable 接口，用来定义 Executor 中线程所执行的任务，主要就是调用 Http11Processor 组件来处理请求
    protected class SocketProcessor extends SocketProcessorBase<NioChannel> {
        ...
        protected void doRun() {
            NioChannel socket = socketWrapper.getSocket(); //获取socket
