...
public class Catalina {
    ...
    public void start() {
        ...
        //用await方法监听停止请求,在 await 方法里会创建一个 Socket 监听 8005 端口，并在一个死循环里接收 Socket 上的连接请求，如果有新的连接到来就建立连接，然后从 Socket 中读取数据；如果读到的数据是停止命令“SHUTDOWN”，就退出循环，进入 stop 流程
        if (await) {
            await();
            stop();
        }
    ...
    protected class CatalinaShutdownHook extends Thread {
        ...
        public void run() {
            try {
                if (getServer() != null) {
                    //执行了 Server 的 stop 方法，Server 的 stop 方法会释放和清理所有的资源
                    Catalina.this.stop();
                }
            } catch (Throwable ex) {
