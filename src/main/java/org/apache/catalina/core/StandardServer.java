...
public final class StandardServer extends LifecycleMBeanBase implements Server {
     ...
     */
    //并没有一开始就分配一个很长的数组，而是在添加的过程中动态地扩展数组长度，当添加一个新的 Service 实例时，会创建一个新数组并把原来数组内容复制到新数组，这样做的目的其实是为了节省内存空间
    //添加一个 Service 到数组中
    @Override
        ...
        try {
            ...
            while (!stopAwait) {
                ...
                try {
                    ...
                    int expected = 1024; // Cut off to avoid DoS attack //截断字符串以避免DDOS攻击
                    while (expected < shutdown.length()) { //如果 自定义了shutdown命令，并且 自定义的shutdown长度大于1024，则
                        if (random == null)
                            random = new Random();
                    ...
                    while (expected > 0) {
                        ...
                        if (ch < 32 || ch == 127) { //遇到控制字符 或者 -1 就退出循环
                            break;
                        }
                ...
                boolean match = command.toString().equals(shutdown); //是否是shutdown命令
                if (match) { //如果是shutdown命令，则退出循环
                    log.info(sm.getString("standardServer.shutdownViaPort"));
                    break;
                } else //如果不是shutdown命令，则再次循环
