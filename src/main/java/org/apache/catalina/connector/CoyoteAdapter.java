        ...
        try {
            ...
            if (postParseSuccess) {
                ...
                request.setAsyncSupported(
                        connector.getService().getContainer().getPipeline().isAsyncSupported());//调用容器的service方法处理请求
                // Calling the container
                connector.getService().getContainer().getPipeline().getFirst().invoke(
                        request, response);
            }
            if (request.isAsync()) {//如果是异步Servlet请求，仅仅设置一个标志
                async = true;
                ReadListener readListener = req.getReadListener();
            ...
            } else {//如果是同步Servlet请求，就将响应数据刷到浏览器
                request.finishRequest();
                response.finishResponse();
            }
